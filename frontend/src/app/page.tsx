...
const rareFeatures = [
    ...
    {
        icon: Fingerprint,
        title: 'SPIRE mTLS, Dual Trust Domains',
        desc: 'Every workload gets a cryptographic identity via SPIFFE/SPIRE — platform and ecosystem trust domains with Envoy sidecars. Single-use join tokens, hourly shadow-decoy guards. Service-to-service auth without shared secrets.',
        stat: '2 trust domains',
        color: 'from-sky-500 to-blue-500',
        bg: 'bg-sky-500/10',
    },
    {
        icon: ScanLine,
        title: 'WAF Shadow Parity',
        desc: 'open-appsec WAF runs on a loopback shadow port in detect-learn mode — every filter verdict is proven byte-identical against direct traffic before any edge cutover. Enabled by default on 8GB+ hosts, auto-off below.',
        stat: 'Byte-identical',
    ...
    {
        icon: Cpu,
        title: 'Idle-Minimal, Burst-Allowed',
        desc: 'Burst workers idle-stop on empty queues, KSM merges identical Python pages, zram compresses cold memory, and every default is sized from your hardware. Heavy load consumes freely — idle sips ~3.5GB.',
        stat: '~3.5GB idle',
...
const comparisons = [
    {
        ...
        features: [
            'Run on your own VPS',
            'PostgreSQL HA (Patroni + PgCat)',
            'Redis Sentinel failover',
            'AI predictive autoscaling',
            'Multi-Git provider support',
            'WireGuard VPN mesh',
            'Custom addon bundles (40+ engines)',
            'Docker socket isolation',
            'Zero-downtime server transfers',
            'Zero-trust node attestation',
            'Inter-server TLS (STRICT mode)',
            'Fail-closed security model',
            'AI Senate Committee (17 providers)',
            'Jules auto-fix loop',
            'SSRF Guard + DNS rebinding protection',
            'open-appsec WAF shadow',
            'SPIRE mTLS (dual trust domains)',
            'CrowdSec WAF + Cloudflare bouncer',
            'Falco eBPF runtime',
    ...
    },
    {
        ...
        features: [
            ...
            'No node attestation',
            'No inter-server TLS',
            'No fail-closed model',
            'No AI remediation',
            'No auto-fix loop',
            'No SSRF protection',
            'No WAF shadow parity',
            'No mTLS workload identity',
            'No runtime syscall monitoring',
            'No self-hosted secrets vault',
...
// ============================================
// COMPLETE STACK — Heavyweight by design, lean at idle
// ============================================
// Every capability below ships ON by default in a fresh install (only
// credentials-gated items — SSL certs, DNS tokens, API keys — need input).
const stackInventory = [
    {
        ...
        color: 'from-blue-500 to-cyan-500',
        bg: 'bg-blue-500/10',
        items: ['PostgreSQL HA (Patroni + PgCat pooling)', 'Redis Sentinel quorum failover', 'RabbitMQ work queues', 'Streaming read replicas'],
    },
    {
        icon: Globe,
        group: 'Edge & Network',
        color: 'from-indigo-500 to-blue-500',
        bg: 'bg-indigo-500/10',
        items: ['Caddy (ACME + on-demand TLS)', 'Traefik dynamic routing', 'WireGuard VPN mesh', 'FRP dev tunnels', 'Cloudflare edge shield'],
    },
    {
        icon: ShieldCheck,
        group: 'Security (8 layers)',
        color: 'from-red-500 to-rose-500',
        bg: 'bg-red-500/10',
        items: ['open-appsec WAF shadow', 'CrowdSec behavioral WAF + CF bouncer', 'SPIRE mTLS (dual trust domains)', 'Falco eBPF runtime', 'Trivy + Cosign every deploy', 'Infisical secrets vault', 'gVisor + AppArmor + fail2ban'],
    },
    {
        icon: Activity,
        group: 'Observability',
        color: 'from-amber-500 to-orange-500',
        bg: 'bg-amber-500/10',
        items: ['Loki log aggregation', 'Prometheus + Alertmanager', 'Grafana dashboards + embeds', 'cAdvisor + Node exporter'],
    },
    {
        icon: Cpu,
        group: 'Compute',
        color: 'from-violet-500 to-purple-500',
        bg: 'bg-violet-500/10',
        items: ['ASGI + websockets (terminal, live logs)', 'Celery burst autoscale (idle-stop)', 'Beat scheduler + redbeat', 'apt + npm build caches'],
    },
    {
        icon: Zap,
        group: 'Efficiency',
        color: 'from-lime-500 to-emerald-500',
        bg: 'bg-lime-500/10',
        items: ['KSM page merging', 'zram compressed swap', 'Hardware-sized defaults', 'Hourly integrity self-heal'],
    },
];

const stackHonesty = [
    { value: '~53', label: 'Services', sub: 'one compose project' },
    { value: '~3.5GB', label: 'Idle RAM', sub: 'bursts stopped, KSM on' },
    { value: '8GB+', label: 'Recommended', sub: '4GB workable' },
    { value: '100%', label: 'Enabled', sub: 'by default, not tiers' },
    { value: '0', label: 'Hidden Fees', sub: 'no per-seat, no add-on fees' },
    { value: '∞', label: 'Features', sub: 'all on by default' },
];
