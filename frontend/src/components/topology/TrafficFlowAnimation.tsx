...
export function TrafficFlowAnimation({ edges }: TrafficFlowAnimationProps) {
  ...
  const getEdgePath = useCallback((edge: Edge): { x1: number; y1: number; x2: number; y2: number } | null => {
    // We need to extract source/target node positions from the DOM
    // ReactFlow stores node positions internally — we query the DOM
