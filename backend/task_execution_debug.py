...
try:
    ...
    if domains.exists():
        ...
        print(f"3. Running DNS verification...")
        ...
        if result.verified:
            print(f"4. DNS is verified - updating domain to dns_verified")
            ...
            if old_status not in ['dns_verified', 'ssl_provisioning', 'active']:
                print(f"   This should trigger Caddy reload")
                print(f"   However, Caddy reload is not happening in this test")
        else:
            print(f"4. DNS is not verified - updating domain to dns_pending")
            domain_obj.status = 'dns_pending'
...
try:
    ...
    if test_domain in caddyfile_content:
        ...
        print(f"Test domain should NOT be included in generated Caddyfile")
...
try:
    ...
    if task_name in app.tasks:
        print(f"✓ Domain task is registered")
        task = app.tasks[task_name]
        print(f"Task info: {task}")
    else:
        print(f"✗ Domain task is NOT registered")
