    ...
    if use_ssl and domain:
        ...
        platform_block.extend(
            [
                ...
                "    # OAuth callbacks come in via two separate paths:",
                "    #",
                "    # 1. /auth/<provider>/callback  →  Settings UI 'Connect' flow.",
                "    #    The Next.js frontend has pages at app/auth/<provider>/callback/",
                "    #    that extract the OAuth code from the URL and POST it to the",
                "    #    backend's /api/v1/integrations/... endpoint. These MUST go to",
                "    #    the FRONTEND (port 3000), not the backend.",
                "    #",
                "    # 2. /auth/<provider>/login     →  django-allauth login initiation.",
                "    #    We rewrite these to /accounts/<provider>/login/ so allauth",
                "    #    can issue the OAuth authorize redirect. The login CALLBACK",
                "    #    uses the standard /accounts/<provider>/login/callback/ path",
                "    #    (already routed to backend by handle /accounts/<provider>/*",
                "    #    below), so no rewrite is needed for the callback.",
                "    #",
                "    # 3. /auth/* catch-all           →  backend (session-token exchange,",
                "    #    AuthProvider /auth/user/ check, etc.)",
