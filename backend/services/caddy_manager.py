    ...
    if use_ssl and domain:
        ...
        platform_block.extend(
            [
                ...
                "    #",
                "    # REWRITE GOTCHA: in Caddy, ``{path}`` inside a rewrite TARGET",
                "    # expands to the full original request path. So",
                "    # ``rewrite * /accounts/github/login/callback{path}`` would turn",
                "    # ``/auth/github/callback`` into",
                "    # ``/accounts/github/login/callback/auth/github/callback``.",
                "    # Use a literal target for the callback (the query string is",
                "    # preserved automatically), and ``uri strip_prefix`` for the",
                "    # login init so ``/auth/github/login`` cleanly becomes",
                "    # ``/accounts/github/login``.",
                "    @oauth_github  path /auth/github/callback*",
                "    @oauth_google  path /auth/google/callback*",
                "    @oauth_gitlab  path /auth/gitlab/callback* /auth/gitlab_oauth2/callback*",
                "    @oauth_bitbucket path /auth/bitbucket/callback* /auth/bitbucket_oauth2/callback*",
                "    @oauth_login   path /auth/github/login* /auth/google/login* /auth/gitlab/login* /auth/gitlab_oauth2/login* /auth/bitbucket/login* /auth/bitbucket_oauth2/login*",
                "    handle @oauth_github {",
