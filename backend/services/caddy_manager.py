    ...
    if use_ssl and domain:
        ...
        platform_block.extend(
            [
                ...
                "    # OAuth callbacks: /auth/<provider>/callback goes to the BACKEND.",
                "    # django-allauth sends this as the OAuth redirect_uri to the",
                "    # provider (GitHub, Google, etc.) so the registered OAuth app",
                "    # callback URL only needs to be set once.  Allauth's callback",
                "    # view at /accounts/<provider>/login/callback/ handles the",
                "    # exchange internally — Caddy just needs to forward the path",
                "    # verbatim; allauth resolves the view from the URL pattern,",
                "    # not from the proxied path.",
                "    #",
                "    # Login init: /auth/<provider>/login rewrites to the allauth",
                "    # login URL so the OAuth dance starts cleanly.",
