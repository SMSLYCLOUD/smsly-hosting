    ...
    if use_ssl and domain:
        ...
        platform_block.extend(
            [
                ...
                "    # OAuth callbacks come in as /auth/<provider>/callback* (the URL",
                "    # the operator pastes into GitHub/Google/GitLab/Bitbucket OAuth",
                "    # app settings). django-allauth serves them at",
                "    # /accounts/<provider>/login/callback/ — so we rewrite before",
                "    # proxying. Order matters: the named matchers must run BEFORE",
                "    # the catch-all /auth/* handle below.",
                "    #",
                "    # The rewrite target must be a LITERAL path (no {path} or {uri}",
                "    # placeholders) because Caddy's placeholder expansion inside a",
                "    # rewrite TARGET appends the full original path, not a suffix.",
                "    # Trailing / is required to avoid a pointless django APPEND_SLASH",
                "    # round trip on every callback.",
                "    @oauth_github_cb   path /auth/github/callback*",
                "    @oauth_google_cb   path /auth/google/callback*",
                "    @oauth_gitlab_cb   path /auth/gitlab/callback* /auth/gitlab_oauth2/callback*",
                "    @oauth_bitbucket_cb path /auth/bitbucket/callback* /auth/bitbucket_oauth2/callback*",
                "    # Login init uses separate explicit matchers per provider so the",
                "    # rewrite target is a static literal (no placeholder expansion).",
                "    @oauth_github_login  path /auth/github/login*",
                "    @oauth_google_login  path /auth/google/login*",
                "    @oauth_gitlab_login  path /auth/gitlab/login* /auth/gitlab_oauth2/login*",
                "    @oauth_bitbucket_login path /auth/bitbucket/login* /auth/bitbucket_oauth2/login*",
                "    handle @oauth_github_cb {",
                "        rewrite * /accounts/github/login/callback/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_google_cb {",
                "        rewrite * /accounts/google/login/callback/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_gitlab_cb {",
                "        rewrite * /accounts/gitlab/login/callback/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_bitbucket_cb {",
                "        rewrite * /accounts/bitbucket_oauth2/login/callback/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_github_login {",
                "        rewrite * /accounts/github/login/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_google_login {",
                "        rewrite * /accounts/google/login/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_gitlab_login {",
                "        rewrite * /accounts/gitlab/login/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle @oauth_bitbucket_login {",
                "        rewrite * /accounts/bitbucket_oauth2/login/",
                "        reverse_proxy backend:8000",
                "    }",
                "    handle /auth/* {",
                "        reverse_proxy backend:8000",
