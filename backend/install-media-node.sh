...
# Phase 2.6: Build voice/video APIs from the master-staged private sources.
# This is deliberately before systemd unit deployment/startup so a clean
# node cannot report success while its API binaries are missing.
if [ "$(state_get phase2c)" != "done" ]; then
    echo -e "\n${BLUE}Phase 2.6: Building voice/video services${NC}"
