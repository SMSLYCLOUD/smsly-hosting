...
select_management_database() {
    ...
    if python manage.py shell -c "from django.conf import settings; exit(0 if 'session' in settings.DATABASES else 1)" ; then
...
create_admin_if_configured() {
    ...
    has_superuser="$(python manage.py shell -c "from django.contrib.auth import get_user_model; User=get_user_model(); print('yes' if User.objects.using('${migrate_db}').filter(is_superuser=True).exists() else 'no')"  | tail -n 1 || true)"
...
setup_social_apps_nonfatal() {
    ...
    python manage.py setup_social_apps --database="$migrate_db"  || \
...
ensure_prometheus_targets_writable() {
    ...
    mkdir -p /opt/smsly-hosting/prometheus-targets  || true



    chown 1000:1000 /opt/smsly-hosting/prometheus-targets  || true
    ...
    find /opt/smsly-hosting/prometheus-targets -type f -exec chmod 666 {} +  || true
    ...
    if ! touch /opt/smsly-hosting/prometheus-targets/.perm_probe ; then
...
ensure_auth_writable() {
    ...
    if [ -d /auth ]; then
        ...
        chown -R 1000:1000 /auth  || true



        chmod -R u+rwX,g+rwX /auth  || true
