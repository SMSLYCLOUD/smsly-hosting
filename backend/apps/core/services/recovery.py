...
Uses a BIP39-style wordlist for the 12-word mnemonic phrase that serves
as a last-resort account recovery mechanism.
"""
import hashlib
import hmac
import secrets
...
BIP39_WORDS = [
    ...
    "alert", "alien", "all", "alley", "allow", "almost", "alone", "along",
    "alphabet", "already", "also", "alter", "always", "amaze", "among", "amount",
    ...
    "bind", "biology", "bird", "birth", "bitter", "black", "blame", "blanket",
    ...
    "call", "calm", "camera", "camp", "can", "canal", "cancel", "candle",
    ...
    "player", "pleasure", "plenty", "plot", "pluck", "plug", "plunge", "poem",
...
def hash_recovery_phrase(phrase: list[str], salt: str = '') -> str:
    """Return a salted SHA-256 hash of the recovery phrase (for storage)."""
    raw = ' '.join(phrase).lower().strip()
    return hashlib.sha256(f"{salt}:{raw}".encode()).hexdigest()


def verify_recovery_phrase(phrase: list[str], stored_hash: str, salt: str = '') -> bool:
    """Verify a recovery phrase against a stored hash."""
    return hmac.compare_digest(hash_recovery_phrase(phrase, salt), stored_hash)


def generate_recovery_salt() -> str:
    """Generate a random salt for recovery phrase hashing."""
