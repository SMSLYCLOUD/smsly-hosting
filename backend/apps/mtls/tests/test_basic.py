...
class MtlsConfigModelTests(TestCase):
    def setUp(self):
        ...
        # The post_save signal auto-creates the row (disabled by default).
        self.config = MtlsConfig.objects.get(service=self.service)

    def test_signal_creates_disabled_config_for_user_services(self):
        """mTLS is OFF by default for non-ecosystem services."""
        self.assertFalse(self.config.enabled)
        self.assertFalse(self.config.sidecar_enabled)
        self.assertEqual(self.config.trust_domain, 'platform.local')

    def test_signal_keeps_mesh_enabled_for_platform_services(self):
        platform_svc = Service.objects.create(
            name='platform-svc', owner=self.user, deploy_type='DOCKER',
            managed_by='PLATFORM',
        )
        cfg = MtlsConfig.objects.get(service=platform_svc)
        self.assertTrue(cfg.enabled)
        self.assertTrue(cfg.sidecar_enabled)

    def test_spiffe_id_auto_generated(self):
        self.assertEqual(
            self.config.spiffe_id, 'spiffe://platform.local/service/test-svc'
        )
        self.config.trust_domain = 'ecosystem.local'
        self.config.save()
        self.assertEqual(
            self.config.spiffe_id, 'spiffe://ecosystem.local/service/test-svc'
        )

    def test_custom_trust_domain(self):
        # Non-allowlisted trust domains are rejected by model validation
        # (save() runs full_clean): user services are pinned to the
        # platform trust domains.
        self.config.trust_domain = 'custom.example.com'
        with self.assertRaises(ValidationError):
            self.config.save()

    def test_is_svid_expired_true_when_no_expiry(self):
        self.config.enabled = True
        self.config.svid_expiry = None
        self.config.save()
        self.assertTrue(self.config.is_svid_expired)

    def test_is_svid_expired_true_when_past(self):
        self.config.enabled = True
        self.config.svid_expiry = timezone.now() - timezone.timedelta(hours=1)
        self.config.save()
        self.assertTrue(self.config.is_svid_expired)
    ...
    def test_is_svid_expired_false_when_future(self):
        ...
        self.config.svid_expiry = timezone.now() + timezone.timedelta(hours=1)
        self.config.save()
        self.assertFalse(self.config.is_svid_expired)

    def test_svid_ttl_remaining_zero_when_no_expiry(self):
        self.config.svid_expiry = None
        self.config.save()
        self.assertEqual(self.config.svid_ttl_remaining, 0)

    def test_svid_ttl_remaining_positive_when_future(self):
        self.config.svid_expiry = timezone.now() + timezone.timedelta(hours=2)
        self.config.save()
        self.assertGreater(self.config.svid_ttl_remaining, 0)
...
class MtlsStatusApiTests(TestCase):
    ...
    def test_mtls_status_returns_config(self):
        ...
        config = MtlsConfig.objects.get(service=self.service)
        config.enabled = True
        config.save()
        resp = self.client.get(f'/api/v1/services/{self.service.id}/mtls/status/')
