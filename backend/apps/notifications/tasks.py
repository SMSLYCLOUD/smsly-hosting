...
def _dispatch_email(user, title: str, message: str, metadata: dict) -> dict:
    ...
    try:
        html_body = (
            ...
            f"CloudNeuron by SMSLY | {timezone.now().strftime('%Y-%m-%d %H:%M UTC')}"
            f"</p></body></html>"
        )
        email = EmailMultiAlternatives(
            subject=f"[CloudNeuron] {title}",
...
def notify_ssl_expiring(user_id: int, domain: str, days_remaining: int):
    ...
    dispatch_notification.delay(
        ...
        message=(
            ...
            f"CloudNeuron will attempt automatic renewal via Let's Encrypt. "
