...
class UsageViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request):
        ...
        quota = {
            'services': meter.check_quota(request.user, 'SERVICE'),
            'cpu': meter.check_quota(request.user, 'CPU'),
        }
        summary['quota'] = {
            'services_remaining': quota['services'][1] if quota['services'] else 0,
            'cpu_remaining': quota['cpu'][1] if quota['cpu'] else 0,
        }
...
from .analytics import AnalyticsPayloadSerializer, AnalyticsViewSet
