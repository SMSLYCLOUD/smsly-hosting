...
def auto_authenticate_nodes_task():
    ...
    Skips immediately if no ManagedServer records exist.
    """
    from apps.deployments.models import ManagedServer

    if not ManagedServer.objects.exists():
        return 0
    ...
    for server in servers:
        if server.ssh_key or server.ssh_password:
            try:
                ...
                logger.warning("Auto-Auth Task timed out for %s (soft 120s)", server.host)
...
def check_managed_servers_health_task():
    ...
    from apps.deployments.views.server.helpers import _refresh_managed_server_health
...
def refresh_managed_server_health(self, server_id: str):
    ...
    from .views.server.helpers import _refresh_managed_server_health
    try:
...
def sync_master_db_to_agents_task():
    ...
    try:
        ...
        from apps.deployments.services.grpc_sync.client import push_db_to_agent
