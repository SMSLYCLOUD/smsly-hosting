...
class GitManager:
    ...
    @staticmethod
    def clone_repo(repo_url: str, branch: str = 'main', destination: str = '/tmp/builds', token: str = None) -> str:
        """
        ...
        if token:
            ...
            authed_url = parsed._replace(netloc=f"oauth2:{token}@{parsed.netloc}").geturl()
        else:
            authed_url = repo_url

        # Clean destination
        repo_dir = os.path.join(destination, repo_url.split('/')[-1].replace('.git', ''))
