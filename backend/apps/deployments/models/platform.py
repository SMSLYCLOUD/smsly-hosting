...
class PlatformConfig(models.Model):
    ...
    # ── CrowdSec Cloudflare bouncer (edge enforcement) ─────────────────
    # Pushes bans to Cloudflare account IP lists so attackers are dropped
    # at the CDN before reaching the origin. Traefik-plugin enforcement
    # stays as defense-in-depth for direct-to-origin traffic.
    crowdsec_cf_enabled = models.BooleanField(  # type: ignore[var-annotated]
        default=False,
        help_text="Push CrowdSec bans to Cloudflare (account-level IP "
                  "lists). Requires an API token + account ID below.")
    crowdsec_cf_api_token = EncryptedCharField(
        max_length=256, blank=True, default='',
        help_text="Cloudflare custom API token with account Firewall/IP "
                  "List write access. Falls back to CROWDSEC_CF_API_TOKEN "
                  "env var if empty.")
    crowdsec_cf_account_id = models.CharField(  # type: ignore[var-annotated]
        max_length=64, blank=True, default='',
        help_text="Cloudflare account ID (32 hex chars, from the dashboard "
                  "URL). Falls back to CROWDSEC_CF_ACCOUNT_ID env var.")
    crowdsec_cf_action = models.CharField(  # type: ignore[var-annotated]
        max_length=32, default='block',
        help_text="Cloudflare remediation for pushed bans: 'block' drops "
                  "at the edge (harshest), 'managed_challenge' issues an "
                  "interactive challenge.")
    crowdsec_cf_bouncer_key = EncryptedCharField(
        max_length=256, blank=True, default='',
        help_text="LAPI key for the cloudflare-bouncer (auto-generated by "
                  "the installer into CROWDSEC_CF_BOUNCER_KEY if empty).")
    ...
    _CONFIG_MAP = {
        ...
        'crowdsec_cf_api_token': ('CROWDSEC_CF_API_TOKEN', ''),
