...
def remove_service_docker_volumes_on_delete(_sender, instance, **kwargs):
    ...
    try:
        ...
        logger.warning("Docker SDK unavailable on service delete: %s", exc)
        return
    for vol in volumes:
        try:
            docker_vol = client.volumes.get(vol.name)
            docker_vol.remove(force=True)
            logger.info("Removed docker volume %s for service %s", vol.name, instance.name)
        except Exception as exc:
            logger.debug("Failed to remove docker volume %s: %s", vol.name, exc)
...
def deprovision_addon_on_delete(_sender, instance, **kwargs):
    ...
    try:
        ...
        docker_vol.remove(force=True)
        logger.info("Removed addon docker volume %s on pre_delete", volume_name)
    except Exception as exc:
        logger.debug("Failed to remove addon docker volume %s: %s", volume_name, exc)
...
def cleanup_managed_server_artifacts(_sender, instance, **kwargs):
    ...
    try:
        ...
        logger.warning("WG peer cleanup failed for server %s: %s", instance.id, exc)

    try:
        if instance.host:
            _cleanup_server_firewall_rules(instance)
    except Exception as exc:
        logger.warning("Firewall cleanup failed for server %s: %s", instance.id, exc)

    try:
        if getattr(instance, "is_lite_agent", False):
            _drop_server_db_user(instance)
    except Exception as exc:
        logger.warning("DB user drop failed for server %s: %s", instance.id, exc)

    try:
        if not getattr(instance, "is_lite_agent", False) and not getattr(instance, "is_primary", False):
            _cleanup_server_dns_record(instance)
    except Exception as exc:
        logger.warning("DNS cleanup failed for server %s: %s", instance.id, exc)

    logger.info("Cleanup finished for server %s", instance.id)
...
def _drop_server_db_user(server):
    ...
    try:
        ...
        logger.info("Dropped DB user %s for server %s", node_db_user, server.id)
    except Exception as exc:
        logger.warning("Failed to drop DB user %s: %s", node_db_user, exc)
...
def _cleanup_server_dns_record(server):
    ...
    try:
        if delete_dns_record(node_domain, cf_token):
            logger.info("Deleted DNS record %s for server %s", node_domain, server.id)
        else:
            logger.warning("Failed to delete DNS record %s for server %s", node_domain, server.id)
    except Exception as exc:
        logger.warning("DNS cleanup error for %s: %s", node_domain, exc)
