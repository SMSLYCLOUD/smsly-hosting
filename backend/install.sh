...
configure_docker_mirror() {
    ...
    if command -v docker  && systemctl is-active --quiet docker; then
        ...
        if ! docker run --rm "$test_img" nslookup registry.npmjs.org ; then
...
EOF
        ...
        if [ -f "$compose_f" ] && grep -q "docker-mirror:" "$compose_f"; then
            ...
            docker compose -f "$compose_f" up -d docker-mirror  || true
...
verify_agent_lite_connectivity() {
    ...
    if ! timeout 2 bash -c "</dev/tcp/${db_check_ip}/5432" ; then
    ...
    if ! timeout 2 bash -c "</dev/tcp/${registry_check_ip}/5000" ; then
    ...
    if [ "${MODE_AGENT_LITE:-false}" = "true" ]; then
        ...
        if [ -z "${MASTER_WG_PUBKEY:-}" ]; then
            ...
            if [ -f "$env_file" ]; then
                ...
                MASTER_WG_PUBKEY="$(env_get_value "$env_file" "MASTER_WG_PUBKEY"  || true)"
            ...
            if [ -z "${MASTER_WG_PUBKEY:-}" ] && [ -f "/opt/smsly-hosting/.agent_lite_seed" ]; then
                ...
                MASTER_WG_PUBKEY="$(env_get_value "/opt/smsly-hosting/.agent_lite_seed" "MASTER_WG_PUBKEY"  || true)"
            ...
            if [ -z "${MASTER_WG_PUBKEY:-}" ] && [ -f "/etc/wireguard/public.key" ]; then
                ...
                MASTER_WG_PUBKEY="$(cat /etc/wireguard/public.key  || true)"
...
run_backend_migrations() {
    ...
    if [ "$MODE_AGENT_LITE" != "true" ] && docker compose -f "$COMPOSE_FILE" ps pgcat  | grep -q "Up"; then
        ...
        timeout 20 docker compose -f "$COMPOSE_FILE" restart pgcat 
...
if [ "${FIX_DOMAIN_MODE:-false}" = "true" ]; then
    ...
    if [ -z "$FIX_DOMAIN" ]; then
        ...
        FIX_DOMAIN="$(env_get_value "$INSTALL_DIR/.env" "DOMAIN"  || true)"
...
ensure_update_networks() {
    ...
    docker network inspect smsly-net  || docker network create smsly-net  || true



    docker network inspect smsly-proxy  || docker network create smsly-proxy  || true



    docker network inspect socket-proxy  || docker network create --driver bridge --internal socket-proxy  || true
...
stop_node_excluded_services() {
    ...
    docker compose -f "$COMPOSE_FILE" stop --timeout 15 frontend caddy  || true



    docker compose -f "$COMPOSE_FILE" rm -f frontend caddy  || true
...
ensure_infrastructure_permissions() {
    ...
    chown -R 1000:1000 "$caddy_config_dir" "$staticfiles_dir" "$builds_dir" "$prometheus_targets_dir"  || true
    ...
    chmod -R u+rwX,g+rwX "$caddy_config_dir" "$staticfiles_dir" "$builds_dir" "$prometheus_targets_dir"  || true
    ...
    find "$prometheus_targets_dir" -type d -exec chmod 2777 {} +  || true
    ...
    chmod 2777 "$prometheus_targets_dir"  || true
    ...
    echo "perm-check $(date +%s)" > "$caddy_config_dir/.perm_probe"  || true
...
resolve_container_target() {
    ...
    if docker container inspect "$target" ; then
    ...
    if [ -f "$compose_f" ]; then
        ...
        services="$(docker compose -f "$compose_f" config --services )"
        ...
        if [ -n "$services" ]; then
            ...
            for svc in $services; do
                ...
                if [[ "$target" == *"-${svc}-"* || "$target" == *"_${svc}_"* || "$target" == *"-${svc}" || "$target" == *"_${svc}" || "$target" == "$svc" ]]; then
                    ...
                    cid="$(docker compose -f "$compose_f" ps -q "$svc"  | head -n 1 || true)"
...
ensure_container_on_network() {
    ...
    docker network connect $alias_flag "$network_name" "$container_name"  || true
...
caddy_needs_fix() {
    ...
    if ! timeout 15 docker compose -f "$COMPOSE_FILE" exec -T caddy caddy validate --config /etc/caddy/Caddyfile ; then
...
refresh_runtime_services() {
    ...
    if [ "$MODE_AGENT_LITE" != "true" ]; then
        ...
        if [ -f "infrastructure/docker/docker-compose.observability.yml" ]; then
            ...
            docker compose -f infrastructure/docker/docker-compose.observability.yml up -d  || true
    ...
    systemctl restart smsly-autoscaler  || true
...
sync_agent_lite_rabbitmq_password() {
    ...
    docker compose -f "$COMPOSE_FILE" up -d rabbitmq  || true
...
recover_runtime_stack() {
    ...
    if ! _registry_certs_ok; then
        ...
        if ! _registry_certs_ok; then
            ...
            docker restart smsly-hosting-registry-1  || true
...
if [ -n "$UPDATE_MODE" ]; then
    ...
    if [ -f "$INSTALL_DIR/.env" ]; then
        ...
        chmod 664 "$INSTALL_DIR/.env"  || true
...
if ! is_checkpoint_done "update_git_synced"; then
     ...
     docker_login
      ...
      case "$UPDATE_MODE" in
         ...
         backend)
            ...
            docker compose -f "$COMPOSE_FILE" stop --timeout 15 backend celery celery-deploy celery-fast celery-beat pgcat  || true
            ...
            docker compose -f "$COMPOSE_FILE" up -d --no-deps pgcat  || true
         ...
         half)
            ...
            if [ "$MODE_NODE" != "true" ]; then
                ...
                docker compose -f "$COMPOSE_FILE" up -d --no-deps frontend  || true
            ...
            if [ "$MODE_AGENT_LITE" = "true" ]; then
                ...
                docker compose -f "$COMPOSE_FILE" up -d --force-recreate $restart_svcs  || true
                ...
                docker compose -f "$COMPOSE_FILE" restart $restart_svcs  || true
         ...
         full)
            ...
            for svc in $CORE_SERVICES; do
                ...
                img=$(docker compose -f "$COMPOSE_FILE" config --images  | grep -i "$svc" || true)
    ...
    if [ "$MODE_AGENT_LITE" != "true" ] && [ "$MODE_NODE" != "true" ]; then
        ...
        if [ -f "infrastructure/docker/docker-compose.observability.yml" ]; then
            ...
            docker compose -f infrastructure/docker/docker-compose.observability.yml up -d --build  || true



            docker restart smsly-grafana  || true



            docker restart smsly-docker-labels  || true



            docker restart smsly-promtail  || true
...
" | timeout 30 docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell  || true
...
"  | tr -d '[:space:]' || true)"
    ...
    for _ce in celery celery-deploy celery-fast celery-beat; do
        ...
        if docker compose -f "$COMPOSE_FILE" config --services  | grep -qx "$_ce"; then



            if ! docker compose -f "$COMPOSE_FILE" ps "$_ce"  | grep -q "Up"; then
                ...
                timeout 30 docker compose -f "$COMPOSE_FILE" up -d --force-recreate --no-deps "$_ce"  || true
...
if [ -f "$INSTALL_DIR/.env" ]; then
    ...
    [ -n "${POSTGRES_PASSWORD:-}" ] || POSTGRES_PASSWORD="$(python3 -c "import secrets; print(secrets.token_hex(16))"  || true)"



    [ -n "${REDIS_PASSWORD:-}" ] || REDIS_PASSWORD="$(python3 -c "import secrets; print(secrets.token_hex(16))"  || true)"



    [ -n "${RABBITMQ_PASSWORD:-}" ] || RABBITMQ_PASSWORD="$(python3 -c "import secrets; print(secrets.token_hex(16))"  || true)"



    [ -n "${GATEWAY_SECRET:-}" ] || GATEWAY_SECRET="$(python3 -c "import secrets; print(secrets.token_hex(32))"  || true)"



    [ -n "${GITHUB_WEBHOOK_SECRET:-}" ] || GITHUB_WEBHOOK_SECRET="$(python3 -c "import secrets; print(secrets.token_hex(32))"  || true)"



    [ -n "${AUTOSCALER_API_TOKEN:-}" ] || AUTOSCALER_API_TOKEN="$(python3 -c "import secrets; print(secrets.token_hex(32))"  || true)"



    [ -n "${FRP_AUTH_TOKEN:-}" ] || FRP_AUTH_TOKEN="$(python3 -c "import secrets; print(secrets.token_hex(32))"  || true)"



    [ -n "${PGCAT_ADMIN_PASSWORD:-}" ] || PGCAT_ADMIN_PASSWORD="$(python3 -c "import secrets; print(secrets.token_hex(24))"  || true)"
...
if docker compose -f "$COMPOSE_FILE" ps pgcat ; then
    ...
    timeout 30 docker compose -f "$COMPOSE_FILE" restart pgcat  || true
...
timeout 30 docker compose -f "$COMPOSE_FILE" restart backend  || true
...
sleep 5
    ...
    docker compose -f "$COMPOSE_FILE" stop --timeout 15 ${MIGRATION_STOPPED_SVCS}  || true
  ...
  || true
    ...
    docker compose -f "$COMPOSE_FILE" start ${MIGRATION_STOPPED_SVCS}  || true
...
if command -v iptables ; then
    ...
    _existing_drop_rule="$(iptables -L DOCKER-USER -n  | grep -c 'dpt:5000.*DROP' || true)"
    ...
    iptables -I DOCKER-USER -i lo -p tcp --dport 5000 -j ACCEPT  || true
    ...
    iptables -I DOCKER-USER -s 172.16.0.0/12 -p tcp --dport 5000 -j ACCEPT  || true
    ...
    iptables -I DOCKER-USER -s 10.100.0.0/24 -p tcp --dport 5000 -j ACCEPT  || true
    ...
    if [ -n "${MASTER_MESH_IP:-}" ]; then
        ...
        iptables -I DOCKER-USER -s "${MASTER_MESH_IP}" -p tcp --dport 5000 -j ACCEPT  || true
    ...
    iptables -A DOCKER-USER -p tcp --dport 5000 -j DROP  || true
...
systemctl enable smsly-autoscaler  || true
...
if [ -f "$INSTALL_DIR/scripts/monitor_infra.sh" ]; then
    ...
    systemctl enable smsly-infra-monitor.timer  || true



    systemctl restart smsly-infra-monitor.timer  || true
...
if [ -f "$INSTALL_DIR/scripts/smsly-update-watcher.service" ]; then
    ...
    systemctl enable smsly-update-watcher caddy-watcher  || true



    systemctl restart smsly-update-watcher caddy-watcher  || true
...
if [ -n "$SSH_CONNECTION" ] && command -v ssh-add ; then
    ...
    SSH_KEY_FP=$(ssh-add -l  | head -1 | awk '{print $2}')
...
"  || echo -e "${YELLOW}  ⚠ Could not register SSH device trust (non-critical)${NC}"
