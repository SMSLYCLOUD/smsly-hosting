...
if [ -z "${STY:-}" ] && [ -z "${SKIP_SCREEN:-}" ] && [ "$NON_INTERACTIVE" != "true" ] && [[ "${1:-}" != "--verify" ]] && [[ "${1:-}" != "--verify-fix" ]] && [[ "${1:-}" != "--verify-autofix" ]] && [[ "${1:-}" != "--debug" ]]; then
...
generate_safe_caddyfile() {
    ...
    local domain=""
    domain="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '[:space:]' || true)"" 2>/dev/null | tr -d '\r' || true)"
    if [ -z "$domain" ]; then
    ...
    local svc_blocks=""
    svc_blocks="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '\r' || true)"" 2>/dev/null | tr -d '\r' || true)"
...
ALL_SVC_DOMAINS="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '\r' || true)"" 2>/dev/null | tr -d '\r' || true)"
...
" | docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell 2>/dev/null || true
    ...
    if command -v caddy &> /dev/null; then
        ...
        if [ -z "$CF_TOKEN" ] || [ "$CF_TOKEN" = "fake" ]; then
            DB_TOKEN="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null || true)"" 2>/dev/null | tr -d '\r' || true)"
...
ENVEOF
            ...
            # Discover domain
            cf_domain=""
            cf_domain="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '[:space:]' || true)"" 2>/dev/null | tr -d '\r' || true)"
            if [ -z "$cf_domain" ]; then
            ...
            cf_wildcard_known_hosts="$(docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '\r' || true)"" 2>/dev/null | tr -d '\r' || true)"
...
" 2>/dev/null || true
    ...
    if [ -n "$GIT_CHANGES" ]; then
        ...
        docker compose -f "$COMPOSE_FILE" exec -T backend python manage.py shell -c "<PY_SNIPPET>"
" 2>/dev/null | tr -d '[:space:]' || true)"" 2>/dev/null | tr -d '\r' || true)"
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$ID" != "ubuntu" && "$ID" != "debian" ]]; then
        ...
        if [ -e /dev/tty ] && [ "$NON_INTERACTIVE" != "true" ]; then
             ...
             echo -e "${YELLOW}  [WARN] Automated mode: Continuing automatically...${NC}"
        fi
    fi
fi
...
if [ -f "$INSTALL_DIR/.env" ]; then
    ...
    if [ "${PRESET_USE_SSL}" = "true" ] && [ -n "${PRESET_DOMAIN}" ] && [ -n "${PRESET_ACME_EMAIL}" ]; then
        ...
        echo -e "${YELLOW}  [WARN] Automated mode detected. Defaulting to IP Mode.${NC}"
        MODE_CHOICE=1
    fi
        ...
        if command -v host &> /dev/null; then
            ...
            if [[ "$DETECTED_IP" != "$PUBLIC_IP" && "$DETECTED_IP" != "127.0.0.1" ]]; then
                ...
                if [ -e /dev/tty ] && [ "$NON_INTERACTIVE" != "true" ]; then
                    ...
                    echo -e "${YELLOW}  [WARN] Automated mode: Ignoring DNS mismatch and continuing...${NC}"
                fi
            else
