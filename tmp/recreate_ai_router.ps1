...
sftp=ssh.open_sftp();
f=sftp.file('/root/proxy_server_config.yaml','w'); f.write(config); f.close(); sftp.close()
# recreate ai-router container with config env and mount
cmds = [
  ...
  "docker run -d --name ai-router-b4635e3a --network smsly-net --memory=2g \
     ...
     -e LITELLM_PROXY_CONFIG=/app/proxy_server_config.yaml \
