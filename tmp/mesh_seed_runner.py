...
MESH_CODE = """from django.contrib.auth import get_user_model
...
def main():
    ...
    sftp = ssh.open_sftp()
    with sftp.file("/tmp/mesh_seed.py", "w") as f:
        f.write(MESH_CODE)
    sftp.close()

    # copy into container and run
    ssh.exec_command("docker cp /tmp/mesh_seed.py smsly-hosting-backend-1:/tmp/mesh_seed.py")[0].channel.recv_exit_status()
    stdin, stdout, stderr = ssh.exec_command("docker exec smsly-hosting-backend-1 python manage.py shell < /tmp/mesh_seed.py", timeout=60)
