...
" 2>/dev/null | tr -d '\r' || true)"
    ...
    # :443 catch-all with self-signed cert for IP redirect to HTTP.
    # The cert is generated by ensure_selfsigned_cert() using OpenSSL
    # with the server's IP as a Subject Alternative Name.
    local tls_block=""
    local host_cert_dir="${INSTALL_DIR:-/opt/smsly-hosting}/caddy-config/certs"
    local host_cert="$host_cert_dir/ip.crt"
    local host_key="$host_cert_dir/ip.key"
    # Container paths (Caddy reads from /etc/caddy which is the shared volume)
    local caddy_cert="/etc/caddy/certs/ip.crt"
    local caddy_key="/etc/caddy/certs/ip.key"
