...
try:
    with transaction.atomic():
        ...
        # Unset REDIS_URL again as it might crash if redis isn't healthy for litellm
        EnvironmentVariable.objects.filter(service=svc, key="REDIS_URL").delete()

        # Let's forcefully start it via manual script to bypass Celery queue just in case it's stuck.
        ...
        print("Started stateful container to test chat.")
