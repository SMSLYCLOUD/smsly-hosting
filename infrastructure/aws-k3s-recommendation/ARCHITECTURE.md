# CloudNeuron Self-Managed Kubernetes Architecture

This document provides a comprehensive architectural recommendation for running **CloudNeuron** (and the 60+ user services it manages) on a self-managed, multi-VPS infrastructure, heavily utilizing AWS EC2 for the control plane and primary worker nodes.

## 1. Core Goal
To provide a highly scalable, self-managed orchestration foundation for a PaaS startup expecting sudden traffic spikes. The system needs to allow CloudNeuron (the control plane) to deploy and route traffic to over 60 distinct user-defined services across various VPS hosting providers over an internal network.
...
### A. The Control Plane (CloudNeuron Core)
...
- **Stateful Backing:** PostgreSQL and Redis for CloudNeuron's internal state.
...
- **Taints & Tolerations:** Use K8s taints to ensure CloudNeuron core services only run on the secure control-plane AWS nodes, while user deployments (`deployment` pods) are scheduled onto the distributed VPS worker nodes.

### C. Ingress and Routing
CloudNeuron currently uses Caddy for wildcard SSL and Traefik for internal routing.
...
## 5. Integrating CloudNeuron with Kubernetes

Currently, CloudNeuron uses the Docker SDK (`docker.from_env()`) via a Docker socket proxy to spin up user containers. To migrate to k3s, the deployment pipeline (`backend/apps/deployments/services/pipeline.py` and `tasks.py`) must be adapted.

### The Strategy: "Docker-in-Docker" builder + Kubernetes API Deployer
1. **Building:** Instead of building directly on the host Docker daemon, CloudNeuron should spin up an ephemeral Kubernetes Job using a builder tool like **Kaniko** or **Buildah**. These tools build Docker images directly inside a K8s pod without needing access to a Docker socket (which is a massive security risk in a multi-tenant K8s cluster).
2. **Registry:** Kaniko pushes the built image to your internal Docker Registry (which you can also host in k3s).
3. **Deploying:** CloudNeuron's Python backend will use the `kubernetes` Python client (instead of `docker`). It will generate K8s `Deployment` and `Service` manifests for the user's app and submit them to the k3s API server.
4. **Routing:** CloudNeuron creates an `IngressRoute` (Traefik CRD) or standard `Ingress` object pointing the user's subdomain to their new K8s Service.

## 6. Traffic Spikes & Scaling

- **Pod Autoscaling:** Use the Kubernetes **Horizontal Pod Autoscaler (HPA)**. CloudNeuron can configure HPA objects for user deployments. If a user's service CPU/Memory spikes, K8s automatically spins up more pods for that specific service across your VPS nodes.
...
5. Package CloudNeuron (Django, Next.js, Celery) into a Helm Chart.
6. Refactor CloudNeuron's `pipeline.py` to use K8s API (Deployments) instead of Docker SDK.
