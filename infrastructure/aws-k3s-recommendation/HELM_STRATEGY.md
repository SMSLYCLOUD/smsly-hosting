# CloudNeuron Helm Chart Strategy

Once your k3s infrastructure is provisioned via Terraform, the next step is deploying CloudNeuron itself (the control plane) into the cluster. The industry standard for packaging complex multi-container K8s applications is **Helm**.

This document outlines how to structure a Helm chart (`charts/cloudneuron`) to replicate and enhance the legacy `docker-compose.prod.yml` setup.

## 1. Directory Structure

A standard Helm chart structure for CloudNeuron should look like this:
...
- **Compose:** CloudNeuron used a `socket-proxy` container to talk to the host's Docker daemon to spin up user apps.
- **Kubernetes:** **You must remove the Docker socket proxy.** K3s uses `containerd`, not Docker, and giving a pod root access to the host container runtime is a severe security risk.
  - **Solution:** The CloudNeuron Backend (`pipeline.py`) must be refactored. Instead of using `docker.from_env()`, it must use the **Kubernetes Python Client** (`pip install kubernetes`). CloudNeuron will generate standard K8s `Deployment` and `Service` YAMLs for user apps and submit them to the k3s API server.
...
- **Kubernetes:** Drop Caddy and Nginx. Use the native **Traefik Ingress Controller** (which comes pre-installed in k3s).
  - **Solution:** Create an `Ingress` resource in Helm that defines the routing rules directly:
    ...
    kind: Ingress
    metadata:
      name: {{ include "cloudneuron.fullname" . }}
      annotations:
      ...
      - host: "app.your-paas.com"
        http:
          ...
          - path: /api
            ...
            backend:
              service:
                name: {{ include "cloudneuron.fullname" . }}-backend
                port:
                  number: 8000
          - path: /
            pathType: Prefix
            backend:
              service:
                name: {{ include "cloudneuron.fullname" . }}-frontend
                port:
                  number: 3000
    ```
...
3.  **Install/Upgrade:**
    ```bash
    helm upgrade --install cloudneuron ./charts/cloudneuron \
      --namespace cloudneuron-system \
      --create-namespace \
      -f values.prod.yaml
    ```
