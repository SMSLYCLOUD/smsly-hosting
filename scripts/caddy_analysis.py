...
try:
    ...
    print("6. Caddy Manager Status")
    print("-" * 40)

    try:
        from apps.deployments.services.caddy_manager import _get_service_domain_blocks

        # Get service domain blocks
        service_blocks = _get_service_domain_blocks()
        print(f"Service domain blocks: {len(service_blocks)}")

        for i, block in enumerate(service_blocks):
            print(f"  Block {i+1}: {block[:100]}...")

        # Check if our test domain would be included
        if any(test_domain in block for block in service_blocks):
            print("Test domain would be included in service blocks")
        else:
            print("Test domain would NOT be included in service blocks")
